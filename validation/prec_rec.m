...
cutoff_idx = max(find(thresh>0.0));
fpr = fpr / total_neg;
...
if (plot_pr || plot_roc)
 ...
 new_tpr=[0;tpr];
 new_prec=[1;prec];
 plot(new_tpr, new_prec, style); % add pseudo point to complete curve
 plot(new_tpr(cutoff_idx+1),new_prec(cutoff_idx+1),'r*');
 fprintf('recall:%f,precision:%f\n',new_tpr(cutoff_idx+1),new_prec(cutoff_idx+1));
 xlabel('recall');
 ylabel('precision');
